...
(() => {
  ...
  style.textContent = `
    ...
    .container > .card:nth-child(1) { animation-delay: .04s; }
    .container > .card:nth-child(2) { animation-delay: .11s; }
    .container > .card:nth-child(3) { animation-delay: .18s; }
    .container > .card:nth-child(4) { animation-delay: .25s; }
  ...
  copyButton?.addEventListener("click", async () => {
    ...
    flash(copyButton, ok ? "✓ Đã copy" : "Không thể copy");
    if (ok) notify("Đã copy link profile");
  });
  shareButton?.addEventListener("click", async () => {
    try {
      if (navigator.share) {
        await navigator.share({ title: document.title, text: "Profile của spekco", url: profileUrl });
        notify("Đã mở bảng chia sẻ");
      } else {
        notify((await copyText(profileUrl)) ? "Đã copy link profile" : "Trình duyệt không hỗ trợ chia sẻ");
      }
    } catch (_) {}
  });
  ...
  if (quote) {
    ...
    let i = 0;
    const type = () => {
      quote.textContent = text.slice(0, i++);
      if (i <= text.length) window.setTimeout(type, 95);
    };
